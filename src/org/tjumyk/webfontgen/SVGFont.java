 ...
 */
public class SVGFont implements XMLConstants, SVGConstants, ScriptTags,
		FeatureTags {
	...
	protected static String getContourAsSVGPathData(Glyph glyph,
		...
		while (offset < count) {
			...
			if (offset == 0) {
				sb.append(PATH_MOVE).append(String.valueOf(point.x))
						.append(XML_SPACE).append(String.valueOf(point.y));
			}

			if (point.onCurve && point_plus1.onCurve) {
				if (point_plus1.x == point.x) { // This is a vertical line
					sb.append(PATH_VERTICAL_LINE_TO).append(
							String.valueOf(point_plus1.y));
				} else if (point_plus1.y == point.y) { // This is a horizontal
														// line
					sb.append(PATH_HORIZONTAL_LINE_TO).append(
							String.valueOf(point_plus1.x));
				} else {
				...
				sb.append(PATH_QUAD_TO).append(String.valueOf(point_plus1.x))
						.append(XML_SPACE)
						.append(String.valueOf(point_plus1.y))
						.append(XML_SPACE)
				...
				sb.append(PATH_QUAD_TO)
						...
						.append(String.valueOf(point_plus1.y))
						.append(XML_SPACE)
						.append(String.valueOf(midValue(point_plus1.x,
								point_plus2.x)))
						.append(XML_SPACE)
						.append(String.valueOf(midValue(point_plus1.y,
								point_plus2.y)));
				offset += 2;
			} else if (!point.onCurve && !point_plus1.onCurve) {
	...
	protected static String getSVGFontFaceElement(Font font) {
		StringBuffer sb = new StringBuffer();
		String fontFamily = font.getNameTable().getRecord(
				Table.nameFontFamilyName);
		...
		// <!ELEMENT font-face
		// (%descTitleMetadata;,font-face-src?,definition-src?) >
		// <!ATTLIST font-face
		// %stdAttrs;
		...
		sb.append(XML_OPEN_TAG_START)
				.append(SVG_FONT_FACE_TAG)
				.append(EOL)
				.append(XML_TAB)
				.append(SVG_FONT_FAMILY_ATTRIBUTE)
				.append(XML_EQUAL_QUOT)
				.append(fontFamily)
				.append(QUOT_EOL)
				// .append("  font-family=\"").append(fontFamily).append("\"\r\n")
				.append(XML_TAB)
				.append(SVG_UNITS_PER_EM_ATTRIBUTE)
				.append(XML_EQUAL_QUOT)
				.append(unitsPerEm)
				.append(QUOT_EOL)
				// .append("  units-per-em=\"").append(unitsPerEm).append("\"\r\n")
				.append(XML_TAB)
				.append(SVG_PANOSE_1_ATTRIBUTE)
				.append(XML_EQUAL_QUOT)
				.append(panose)
				.append(QUOT_EOL)
				// .append("  panose-1=\"").append(panose).append("\"\r\n")
				.append(XML_TAB).append(SVG_ASCENT_ATTRIBUTE)
				.append(XML_EQUAL_QUOT)
				.append(ascent)
				.append(QUOT_EOL)
				// .append("  ascent=\"").append(ascent).append("\"\r\n")
				.append(XML_TAB).append(SVG_DESCENT_ATTRIBUTE)
				.append(XML_EQUAL_QUOT).append(descent)
				.append(QUOT_EOL)
				// .append("  descent=\"").append(descent).append("\"\r\n")
				.append(XML_TAB).append(SVG_ALPHABETIC_ATTRIBUTE)
				.append(XML_EQUAL_QUOT).append(baseline).append(XML_CHAR_QUOT)
	 ...
	 * 
	 * @param font
	 *            The TrueType font to be converted to SVG
	 * @param id
	 *            An XML id attribute for the font element
	 * @param first
	 *            The first character in the output range
	 * @param last
	 *            The last character in the output range
	 * @param forceAscii
	 *            Force the use of the ASCII character map
	 */
	protected static void writeFontAsSVGFragment(PrintStream ps, Font font,
			String id, int first, int last, boolean autoRange,
			boolean forceAscii) throws Exception {
		// StringBuffer sb = new StringBuffer();
		...
		if (forceAscii) {
			...
			cmapFmt = font.getCmapTable().getCmapFormat(
					Table.platformMacintosh, Table.encodingRoman);
		} else {
			// The default behaviour is to use the Unicode cmap encoding
			cmapFmt = font.getCmapTable().getCmapFormat(
					Table.platformMicrosoft, Table.encodingUGL);
			if (cmapFmt == null) {
				// This might be a symbol font, so we'll look for an "undefined"
				// encoding
				cmapFmt = font.getCmapTable().getCmapFormat(
						Table.platformMicrosoft, Table.encodingUndefined);
			}
		}
		if (cmapFmt == null) {
		...
		if (gsub != null) {
			...
			if (s != null) {
				...
				if (ls != null) {
					Feature init = gsub.getFeatureList().findFeature(ls,
							FEATURE_TAG_INIT);
					Feature medi = gsub.getFeatureList().findFeature(ls,
							FEATURE_TAG_MEDI);
					Feature fina = gsub.getFeatureList().findFeature(ls,
							FEATURE_TAG_FINA);

					initialSubst = (SingleSubst) gsub.getLookupList()
							.getLookup(init, 0).getSubtable(0);
					medialSubst = (SingleSubst) gsub.getLookupList()
							.getLookup(medi, 0).getSubtable(0);
					terminalSubst = (SingleSubst) gsub.getLookupList()
							.getLookup(fina, 0).getSubtable(0);
				}
			}
		}
			...
			for (int i = first; i <= last; i++) {
				...
				if (glyphIndex > 0) {
					ps.println(getGlyphAsSVG(
							...
							medialSubst,
							terminalSubst,
							(32 <= i && i <= 127) ? encodeEntities(String
									.valueOf((char) i)) : XML_CHAR_REF_PREFIX
	 ...
	 *            An XML id attribute for the font element
	 * @param str
	 *            The input string containing all characters to handle
	 * @param forceAscii
	 *            Force the use of the ASCII character map
	 */
	protected static void writeFontAsSVGFragment(PrintStream ps, Font font,
			String id, String str, boolean forceAscii) throws Exception {
		// StringBuffer sb = new StringBuffer();
		// int horiz_advance_x = font.getHmtxTable().getAdvanceWidth(
		// font.getHheaTable().getNumberOfHMetrics() - 1);
		int horiz_advance_x = font.getOS2Table().getAvgCharWidth();
		...
		try {
			...
			Character[] charArr = (Character[]) chars
					.toArray(new Character[] {});
			Arrays.sort(charArr);
			// Include our requested range
			for (int ind = 0, len = charArr.length; ind < len; ind++) {
				int i = charArr[ind];
				int glyphIndex = cmapFmt.mapCharCode(i);
				// ps.println(String.valueOf(i) + " -> " +
				// String.valueOf(glyphIndex));
	...
	protected static String getGlyphAsSVG(Font font, Glyph glyph,
			int glyphIndex, int defaultHorizAdvanceX, String attrib, String code) {
		...
		if (glyphIndex == 0) {
			...
			// Unicode value
			sb.append(XML_OPEN_TAG_START).append(SVG_GLYPH_TAG)
					.append(XML_SPACE).append(SVG_UNICODE_ATTRIBUTE)
					.append(XML_EQUAL_QUOT).append(code).append(XML_CHAR_QUOT);
			// sb.append("<glyph unicode=\"").append(code).append("\"");

			// Glyph name
			sb.append(XML_SPACE).append(SVG_GLYPH_NAME_ATTRIBUTE)
					.append(XML_EQUAL_QUOT)
		...
		if (horiz_advance_x != defaultHorizAdvanceX) {
			sb.append(XML_SPACE).append(SVG_HORIZ_ADV_X_ATTRIBUTE)
					.append(XML_EQUAL_QUOT).append(horiz_advance_x)
					.append(XML_CHAR_QUOT);
	...
	protected static String getGlyphAsSVG(Font font, Glyph glyph,
			int glyphIndex, int defaultHorizAdvanceX,
			SingleSubst arabInitSubst, SingleSubst arabMediSubst,
			SingleSubst arabTermSubst, String code) {
		...
		if (arabInitGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabInitGlyphIndex),
					arabInitGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"initial\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT
							+ SVG_INITIAL_VALUE + XML_CHAR_QUOT), code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		}
		...
		if (arabMediGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabMediGlyphIndex),
					arabMediGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"medial\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT
							+ SVG_MEDIAL_VALUE + XML_CHAR_QUOT), code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		}
		...
		if (arabTermGlyphIndex != glyphIndex) {
			sb.append(getGlyphAsSVG(font, font.getGlyph(arabTermGlyphIndex),
					arabTermGlyphIndex, defaultHorizAdvanceX,
					// " arabic-form=\"terminal\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT
							+ SVG_TERMINAL_VALUE + XML_CHAR_QUOT), code));
			// sb.append("\r\n");
			sb.append(EOL);
			substituted = true;
		}
		...
		if (substituted) {
			sb.append(getGlyphAsSVG(font, glyph, glyphIndex,
					defaultHorizAdvanceX,
					// " arabic-form=\"isolated\"",
					(XML_SPACE + SVG_ARABIC_FORM_ATTRIBUTE + XML_EQUAL_QUOT
							+ SVG_ISOLATED_VALUE + XML_CHAR_QUOT), code));
		} else {
			sb.append(getGlyphAsSVG(font, glyph, glyphIndex,
					defaultHorizAdvanceX, null, code));
		}
	...
	protected static String getKerningPairAsSVG(KerningPair kp, PostTable post) {
		...
		// sb.append("\" g2=\"");
		sb.append(XML_CHAR_QUOT).append(XML_SPACE).append(SVG_G2_ATTRIBUTE)
				.append(XML_EQUAL_QUOT);

		sb.append(post.getGlyphName(kp.getRight()));
		// sb.append("\" k=\"");
		sb.append(XML_CHAR_QUOT).append(XML_SPACE).append(SVG_K_ATTRIBUTE)
				.append(XML_EQUAL_QUOT);
	...
	/*
	 * protected static String getGlyphAsPath(Glyph glyph) { StringBuffer sb =
	 * new StringBuffer(); int firstIndex = 0; int count = 0; int i;
	 * 
	 * for (i = 0; i < glyph.getPointCount(); i++) { count++; if
	 * (glyph.getPoint(i).endOfContour) {
	 * sb.append(getContourAsSVGPathData(glyph, firstIndex, count)); firstIndex
	 * = i + 1; count = 0; } } return sb.toString(); }
	 * 
	 * protected static void writeTextAsSVGFragment(PrintStream ps, Font f, int
	 * size, String text) { CmapFormat cmapFmt =
	 * f.getCmapTable().getCmapFormat(Table.platformMicrosoft,
	 * Table.encodingUGL); int x = 0; for (short i = 0; i < text.length(); i++)
	 * { int glyphIndex = cmapFmt.mapCharCode((short)text.charAt(i)); Glyph
	 * glyph = f.getGlyph(glyphIndex); if (glyph != null) {
	 * ps.println(translateSVG(x, 0, getGlyphAsSVGPath(glyph))); x +=
	...
	protected static void writeSvgBegin(PrintStream ps) {
		...
		ps.println(Messages.formatMessage(CONFIG_SVG_BEGIN, new Object[] {
				SVG_PUBLIC_ID, SVG_SYSTEM_ID }));

	}

	protected static void writeSvgDefsBegin(PrintStream ps) {
		// ps.println("<defs>");
		ps.println(XML_OPEN_TAG_START + SVG_DEFS_TAG
	...
	protected static void writeSvgTestCard(PrintStream ps, String fontFamily) {
		 ...
		 * "'; font-size:18;fill:black\">"); ps.println(
	...
	public static final String ARG_KEY_STRING = "-s";

	/**
	 * (Old) Starts the application.
	 * 
	 * @param args
	...
	public static void main(String[] args) {
		try {
			String path = parseArgs(args, null);
			String str = parseArgs(args, ARG_KEY_STRING);
			String id = parseArgs(args, ARG_KEY_ID);
			String ascii = parseArgs(args, ARG_KEY_ASCII);
			String testCard = parseArgs(args, ARG_KEY_TESTCARD);
			String outPath = parseArgs(args, ARG_KEY_OUTPUT_PATH);
			...
			if (str == null || str.length() <= 0) {
				StringBuilder sb = new StringBuilder(DEFAULT_LAST
						- DEFAULT_FIRST + 1);
				for (int i = DEFAULT_FIRST; i <= DEFAULT_LAST; i++) {
					sb.append((char) i);
				}
				str = sb.toString();
			}
			...
			if (path != null) {
				...
				writeSvgBegin(ps);
				writeSvgDefsBegin(ps);
				writeFontAsSVGFragment(ps, font, id, str, (ascii != null)); // major
																			// change
				writeSvgDefsEnd(ps);
	...
	private static void chopUpStringBuffer(StringBuffer sb) {
		if (sb.length() < 256) {
			...
			// Being rather simplistic about it, for now we'll insert a newline
			// after
			// 240 chars
	 ...
	 * StringBuffer sb = new StringBuffer();
	 * sb.append("<g transform=\"translate(") .append(String.valueOf(x))
	 * .append(" ") .append(String.valueOf(y)) .append(")\">") .append(svgText)
	 * .append("</g>"); return sb.toString(); }
	 */
	...
	private static String parseArgs(String[] args, String name) {
		for (int i = 0; i < args.length; i++) {
			if (name == null) {
				...
				if ((i < args.length - 1)
						&& (args[i + 1].length() <= 0 || args[i + 1].charAt(0) != ARG_KEY_START_CHAR)) {
					return args[i + 1];
				} else {
					return args[i];
				}
	...
	private static void usage() {
		...
		System.err
				.println("usage: java org.apache.batik.svggen.font.SVGFont <ttf-path> [-s <input-string>] [-ascii] [-id <id>] [-o <output-path>] [-testcard]");
	}
}
